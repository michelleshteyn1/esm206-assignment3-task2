...
sw_split_name<-starwars %>% #starting with 'starwars'
  separate(col = "name", # separate the 'name' column
           into = c("name_1","name_2"), #into two columns, 'name_1' and 'name_2'
           sep = "-") # separate at the first dash

#The name in Row 49, which contains two dashes, drops the 'Mundi' part of the name, because it is after a second dash.
...
sw_skin_sep <- starwars %>% #Start with starwars
  separate(col = skin_color, #Separate the 'skin_color' column
           ...
           sep = ",", #Create the separation at the first comma
           extra = "merge", #If additional commas, merge all into the second column
           remove = FALSE) #And *do not* remove the original column
```
...
mpg_model_class <- mpg %>% #Start with mpg
  unite(col = model_class, #Create new column called 'model_class'
        c(model,class), #Which combines the model and class columns
        sep = "-", #Puts a dash between the pieces
        remove = FALSE) #And do not remove the original column
...
some_dates <- c(11061972, 02142007,01301843) #Creates a vector of dates
...
storms_danger <- storms %>% #start from 'storms
  mutate(danger = case_when( #create a new column called 'danger'
    status=="hurricane"~"high", #"high" if 'status' is "hurricane
    status!="hurricane"~"moderate" #"moderate" if 'status is anything else
  ))

#I was confused by the 'TRUE' argument but the below worked the same as the top
storms_danger1 <- storms %>% 
  mutate(danger = case_when( 
    status=="hurricane"~"high", 
    TRUE~"moderate" #any entry that hasn't been specified by previous conditions will get that 'catch all' statement
  ))
